 ...
 *
 * Player-Identity-Fix (siehe ROADMAP_DEPENDENCIES.md): alle identitätsabhängigen
 * Funktionen nehmen jetzt `myUserId` statt `myPlayerIndex` entgegen und matchen
 * PRO RECORD gegen `player.userId` — nicht mehr gegen einen festen Index. Die
 * Fixtures unten geben dem "ich"-Spieler deshalb explizit eine `userId`
 * (`MY_USER_ID`), statt sich implizit auf Index 0 zu verlassen.
 */
...
/** Baut den "ich"-Spieler mit einer festen, stabilen userId (Auth-Token-Konvention). */
function mePlayer(overrides: Partial<ICmrPlayer> = {}): ICmrPlayer {
  return player({ userId: MY_USER_ID, ...overrides });
}
...
function record(overrides: Partial<ICanonicalMatchResult> = {}): ICanonicalMatchResult {
  return {
    ...
    winnerIndex: 0,
    players: [
      mePlayer({ index: 0, average: 60, total180: 2, dartsThrown: 90, legs: 3, checkoutPoints: 100 }),
...
describe("computeMatchSummary", () => {
  it("1. keine Matches: alles 0, winRate null (keine Division durch 0)", () => {
    const s = computeMatchSummary([], MY_USER_ID);
    assert.deepEqual(s, {
  ...
  it("2. ein gewonnenes Match", () => {
    const s = computeMatchSummary([ record({ winnerIndex: 0 }) ], MY_USER_ID);
  ...
  it("3. ein verlorenes Match", () => {
    const s = computeMatchSummary([ record({ winnerIndex: 1 }) ], MY_USER_ID);
  ...
  it("5. unentschiedenes Match (kein winnerIndex) zählt weder als Sieg noch Niederlage", () => {
    const s = computeMatchSummary([ record({ winnerIndex: undefined, quality: "MINIMAL" }) ], MY_USER_ID);
    assert.equal(s.wins, 0);
    assert.equal(s.losses, 0);
  ...
  it("6. finished=false zählt nicht als completed, auch wenn winnerIndex zufällig gesetzt wäre", () => {
    const s = computeMatchSummary([ record({ finished: false, winnerIndex: 0 }) ], MY_USER_ID);
    assert.equal(s.completedMatches, 0);
    assert.equal(s.decidedMatches, 0);
  });

  it("7. Identität wird PRO MATCH aufgelöst, nicht global auf einen Index fixiert", () => {
    // Derselbe Nutzer sitzt in Match A an Index 0, in Match B an Index 1 — beide
    // Male korrekt als "ich" erkannt, weil über userId statt Index gematcht wird.
    const s = computeMatchSummary([
      record({ matchId: "a", winnerIndex: 0, players: [ mePlayer({ index: 0 }), player({ index: 1 }) ] }),
      record({ matchId: "b", winnerIndex: 1, players: [ player({ index: 0 }), mePlayer({ index: 1 }) ] }),
    ], MY_USER_ID);
    assert.equal(s.wins, 2);
    assert.equal(s.losses, 0);
  });
...
describe("computeAverageStats", () => {
  it("1. keine Matches: alles null", () => {
    const a = computeAverageStats([], MY_USER_ID);
    assert.deepEqual(a, { average: null, bestAverage: null, sampleSize: 0 });
  });

  it("2. ein Match", () => {
    const a = computeAverageStats([ record() ], MY_USER_ID);
  ...
  it("3. mehrere Matches: korrekte Aggregation und Bestwert", () => {
    const a = computeAverageStats([
      record({ matchId: "a", players: [ mePlayer({ index: 0, average: 40 }), player({ index: 1 }) ] }),
      record({ matchId: "b", players: [ mePlayer({ index: 0, average: 80 }), player({ index: 1 }) ] }),
      record({ matchId: "c", players: [ mePlayer({ index: 0, average: 60 }), player({ index: 1 }) ] }),
    ], MY_USER_ID);
  ...
  it("4. fehlendes average-Feld wird übersprungen, nicht als 0 gewertet", () => {
    const a = computeAverageStats([
      record({ matchId: "a", players: [ mePlayer({ index: 0, average: 100 }), player({ index: 1 }) ] }),
      record({ matchId: "b", players: [ mePlayer({ index: 0 }), player({ index: 1 }) ] }), // kein average
    ], MY_USER_ID);
...
describe("computeScoringStats", () => {
  it("1. keine Matches: 0/null-Werte, kein Fehler", () => {
    const s = computeScoringStats([], MY_USER_ID);
  ...
  it("2. total180 wird summiert, avg180PerMatch nur über Matches mit dem Feld gemittelt", () => {
    const s = computeScoringStats([
      record({ matchId: "a", players: [ mePlayer({ index: 0, total180: 3 }), player({ index: 1 }) ] }),
      record({ matchId: "b", players: [ mePlayer({ index: 0, total180: 1 }), player({ index: 1 }) ] }),
    ], MY_USER_ID);
  ...
  it("3. highestCheckout ist das Maximum, nicht die Summe", () => {
    const s = computeScoringStats([
      record({ matchId: "a", players: [ mePlayer({ index: 0, checkoutPoints: 40 }), player({ index: 1 }) ] }),
      record({ matchId: "b", players: [ mePlayer({ index: 0, checkoutPoints: 121 }), player({ index: 1 }) ] }),
      record({ matchId: "c", players: [ mePlayer({ index: 0, checkoutPoints: 90 }), player({ index: 1 }) ] }),
    ], MY_USER_ID);
  ...
  it("4. avgFirst9 nur über Matches, die das Feld führen (nicht jede Variante hat es)", () => {
    const s = computeScoringStats([
      record({ matchId: "a", players: [ mePlayer({ index: 0, first9Average: 90 }), player({ index: 1 }) ] }),
      record({ matchId: "b", players: [ mePlayer({ index: 0 }), player({ index: 1 }) ] }), // kein first9Average
    ], MY_USER_ID);
...
describe("computeLegSetStats", () => {
  it("1. legsWon/legsLost korrekt gegen alle Gegner summiert", () => {
    const s = computeLegSetStats([ record() ], MY_USER_ID); // me: legs=3, opponent: legs=1
    assert.equal(s.legsWon, 3);
    assert.equal(s.legsLost, 1);
  });

  it("2. setsWon/setsLost sind null, wenn keine Variante Sets führt", () => {
    const s = computeLegSetStats([ record() ], MY_USER_ID); // players haben kein sets-Feld
  ...
  it("3. setsWon/setsLost werden gezählt, sobald mindestens ein Match Sets führt", () => {
    const s = computeLegSetStats([
      record({
        players: [
          mePlayer({ index: 0, legs: 3, sets: 2 }),
...
describe("computeAverageTrend", () => {
  it("1. sortiert chronologisch aufsteigend (älteste zuerst)", () => {
    const trend = computeAverageTrend([
      record({ matchId: "neu", recordedAt: "2026-08-17T10:00:00.000Z", players: [ mePlayer({ index: 0, average: 70 }), player({ index: 1 }) ] }),
      record({ matchId: "alt", recordedAt: "2026-08-10T10:00:00.000Z", players: [ mePlayer({ index: 0, average: 50 }), player({ index: 1 }) ] }),
    ], MY_USER_ID);
  ...
  it("2. Matches ohne average werden ausgelassen", () => {
    const trend = computeAverageTrend([
      record({ matchId: "a", players: [ mePlayer({ index: 0 }), player({ index: 1 }) ] }),
    ], MY_USER_ID);
    assert.equal(trend.length, 0);
  });

  it("3. leere Liste ergibt leeren Trend", () => {
    assert.deepEqual(computeAverageTrend([], MY_USER_ID), []);
  });
});
...
describe("computeRecentForm", () => {
  it("1. neueste zuerst, begrenzt auf `count`", () => {
    ...
    const form = computeRecentForm(records, 2, MY_USER_ID);
  ...
  it("2. unentschiedenes Match hat won=null, nicht false", () => {
    const form = computeRecentForm([ record({ winnerIndex: undefined, quality: "MINIMAL" }) ], 10, MY_USER_ID);
...
describe("computeStatisticsOverview (Integration mehrerer Funktionen)", () => {
  it("1. leere Liste: keine Exceptions, alles defensiv null/0", () => {
    const overview = computeStatisticsOverview([], undefined, MY_USER_ID);
  ...
  it("2. gameModes stammen aus dem UNGEFILTERTEN Bestand (Filter-Optionen bleiben vollständig)", () => {
    ...
    const overview = computeStatisticsOverview(records, { period: "all", gameMode: "X01" }, MY_USER_ID);
  ...
  it("3. defensive Behandlung von grob unvollständigen Records (kein players-Crash)", () => {
    ...
    assert.doesNotThrow(() => computeStatisticsOverview([ broken ], undefined, MY_USER_ID));
  });
});

/**
 * Player-Identity-Fix — dedizierte Regressionstests (Phase 2).
 *
 * Deckt genau die im Audit geforderten Szenarien ab: Sieg/Niederlage an
 * beiden Indizes, unbekannte Identität, fehlender Token, und dass
 * nicht-identitätsabhängige Kennzahlen unverändert bleiben.
 */
describe("Player Identity (userId statt Index-0-Annahme)", () => {
  it("CASE A: User = players[0], User gewinnt → Win korrekt", () => {
    const s = computeMatchSummary([
      record({ winnerIndex: 0, players: [ mePlayer({ index: 0 }), player({ index: 1, userId: "opponent" }) ] }),
    ], MY_USER_ID);
    assert.equal(s.wins, 1);
    assert.equal(s.losses, 0);
  });
  ...
  it("CASE B: User = players[1], User gewinnt → Win korrekt", () => {
    const s = computeMatchSummary([
      record({ winnerIndex: 1, players: [ player({ index: 0, userId: "opponent" }), mePlayer({ index: 1 }) ] }),
    ], MY_USER_ID);
    assert.equal(s.wins, 1);
    assert.equal(s.losses, 0);
  });

  it("CASE C: User = players[1], User verliert → Loss korrekt", () => {
    const s = computeMatchSummary([
      record({ winnerIndex: 0, players: [ player({ index: 0, userId: "opponent" }), mePlayer({ index: 1 }) ] }),
    ], MY_USER_ID);
    assert.equal(s.wins, 0);
    assert.equal(s.losses, 1);
  });

  it("CASE D: User = players[0], User verliert → Loss korrekt", () => {
    const s = computeMatchSummary([
      record({ winnerIndex: 1, players: [ mePlayer({ index: 0 }), player({ index: 1, userId: "opponent" }) ] }),
    ], MY_USER_ID);
    assert.equal(s.wins, 0);
    assert.equal(s.losses, 1);
  });
  ...
  it("CASE E: User-ID fehlt in players[] → keine falsche Zuordnung zu Index 0", () => {
    // winnerIndex ist 0, aber "ich" (MY_USER_ID) bin in diesem Match gar nicht dabei
    // (z.B. ein fremdes Match, ein Gast ohne stabile ID, oder ein reines Bot-Match).
    // Der alte Code hätte hier fälschlich einen Sieg gezählt (Index-0-Fallback).
    const s = computeMatchSummary([
      record({ winnerIndex: 0, players: [ player({ index: 0, userId: "stranger-a" }), player({ index: 1, userId: "stranger-b" }) ] }),
    ], MY_USER_ID);
    assert.equal(s.wins, 0);
    assert.equal(s.losses, 0);
    assert.equal(s.decidedMatches, 0, "Match ohne aufgelöste Identität darf nicht als entschieden gezählt werden");

    // Gleiches Prinzip für Recent Form: won muss null sein, nicht false.
  ...
  it("CASE F: kein/ungültiger Auth-Token (myUserId undefined/null/leer) → defensives Verhalten, kein Crash", () => {
    const records = [ record({ winnerIndex: 0 }), record({ matchId: "b", winnerIndex: 1 }) ];

    for (const noId of [ undefined, null, "" ] as const) {
      const s = computeMatchSummary(records, noId);
      assert.equal(s.wins, 0);
      assert.equal(s.losses, 0);
      assert.equal(s.decidedMatches, 0, `myUserId=${JSON.stringify(noId)} darf kein Match werten`);

      const a = computeAverageStats(records, noId);
      assert.equal(a.average, null);
      assert.equal(a.sampleSize, 0);
    }
  });

  it("CASE G: nicht-identitätsabhängige Statistics bleiben unverändert (Quality, Match-Anzahl, Filter)", () => {
    const records = [
      record({ matchId: "a", quality: "COMPLETE" }),
      record({ matchId: "b", quality: "PARTIAL", players: [ player({ index: 0, userId: "stranger" }), player({ index: 1 }) ] }),
    ];

    // Egal ob/welche myUserId übergeben wird — Quality-Breakdown und totalMatches
    // hängen nicht von der Identität ab.
    assert.deepEqual(computeQualityBreakdown(records), { complete: 1, partial: 1, minimal: 0 });

    const withId = computeMatchSummary(records, MY_USER_ID);
    const withoutId = computeMatchSummary(records, undefined);
    assert.equal(withId.totalMatches, 2);
    assert.equal(withoutId.totalMatches, 2);
    assert.equal(withId.completedMatches, withoutId.completedMatches);
  });
});
