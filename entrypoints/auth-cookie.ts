...
export default defineUnlistedScript(() => {
  ...
  // ── Refresh-Trigger: Content-Script kann uns bitten, einen frischen Token
  //    einzufangen (v2.9.90 Login-Race-Fix). Wir stoßen dazu einen leichten
  //    API-Call an, den die Autodarts-App selbst ohnehin ständig macht. Ihr
  //    HTTP-Client hängt einen Bearer-Header an — den fängt unser XHR/fetch-
  //    Interceptor oben ab und dispatched das aktuelle access_token.
  //    Falls die App unseren fetch nicht auth-wrapped, dispatched wir wenigstens
  //    den letzten bekannten Token erneut, damit der Content-Script "wach" wird.
  window.addEventListener("adt-request-token-refresh", () => {
    try {
      console.log("[Auth] Refresh-Trigger empfangen — probe /gs/v0/users/me");
      // `credentials: include` sorgt dafür dass Autodarts-Session-Cookies mit
      // gesendet werden; die App wrapped fetch üblicherweise mit einem Bearer-
      // Interceptor der auf unseren originalFetch geht.
      window.fetch("https://api.autodarts.io/gs/v0/users/me", {
        credentials: "include",
      }).catch(() => { /* harmlos */ });
    } catch (_) { /* ignore */ }
    // Force re-emit des letzten bekannten Tokens (auch identisch), damit
    // der Content-Script GlobalStatus.auth.tokenAt aktualisiert und alte
    // Waiter aufwachen.
    if (lastToken) {
      try {
        window.dispatchEvent(new CustomEvent("auth-cookie-available", {
          detail: { authValue: lastToken, forced: true },
        }));
      } catch (_) { /* ignore */ }
    }
  });
