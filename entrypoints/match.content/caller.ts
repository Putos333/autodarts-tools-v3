...
let config: IConfig;
// Stummschaltung des Autodarts-Original-Callers
let autodartsAudioMuted = false;
let adtCallerObserver: MutationObserver | null = null;
...
let activeCallerName: string | null = null;
// Letzter Match-Identifier um Caller-Wechsel nur bei neuem Match auszulösen
...
export function callerOnRemove() {
  ...
  // Autodarts-Original-Caller wieder aktivieren
  unmuteAutodartsOriginalCaller();
}

/**
 * Autodarts-Original-Caller stummschalten.
 * Autodarts verwendet intern Audio-Elemente die Sounds abspielen.
 * Wir setzen deren volume auf 0 solange unser Caller aktiv ist.
 */
function muteAutodartsOriginalCaller(): void {
  if (autodartsAudioMuted) return;
  try {
    // Alle Audio-Elemente die NICHT von uns stammen stummschalten
    const allAudio = Array.from(document.querySelectorAll("audio")) as HTMLAudioElement[];
    allAudio.forEach((audio) => {
      if (!audio.dataset.adtOwned) {
        audio.volume = 0;
        audio.dataset.adtMuted = "true";
      }
    });
    // MutationObserver: neu hinzugefügte Audio-Elemente sofort stummschalten
    adtCallerObserver = new MutationObserver((mutations) => {
      mutations.forEach((mutation) => {
        mutation.addedNodes.forEach((node) => {
          if (node instanceof HTMLAudioElement && !node.dataset.adtOwned) {
            node.volume = 0;
            node.dataset.adtMuted = "true";
          }
          // Auch Kind-Elemente prüfen
          if (node instanceof Element) {
            node.querySelectorAll("audio:not([data-adt-owned])").forEach((audio) => {
              (audio as HTMLAudioElement).volume = 0;
              (audio as HTMLAudioElement).dataset.adtMuted = "true";
            });
          }
        });
      });
    });
    adtCallerObserver.observe(document.documentElement, { childList: true, subtree: true });
    autodartsAudioMuted = true;
    console.log("Autodarts Tools: Original-Caller stummgeschaltet (", allAudio.length, "Audio-Elemente)");
  } catch (e) {
    console.error("Autodarts Tools: Fehler beim Stummschalten des Original-Callers", e);
  }
}
 ...
 * Autodarts-Original-Caller wieder aktivieren.
 */
function unmuteAutodartsOriginalCaller(): void {
  if (!autodartsAudioMuted) return;
  try {
    const mutedAudio = Array.from(document.querySelectorAll("audio[data-adt-muted]")) as HTMLAudioElement[];
    mutedAudio.forEach((audio) => {
      audio.volume = 1;
      delete audio.dataset.adtMuted;
    });
    if (adtCallerObserver) {
      adtCallerObserver.disconnect();
      adtCallerObserver = null;
    }
    autodartsAudioMuted = false;
    console.log("Autodarts Tools: Original-Caller wieder aktiviert");
  } catch (e) {
    console.error("Autodarts Tools: Fehler beim Reaktivieren des Original-Callers", e);
  }
}
...
function initAudioPlayer(): void {
  if (!audioPlayer) {
    ...
    for (let i = 0; i < AUDIO_POOL_SIZE; i++) {
      ...
      audio.dataset.adtOwned = "true"; // Nicht stummschalten
    ...
    notificationElement.innerHTML = `
      <div class="adt-notification-content">
        <div class="adt-notification-message">
          Bitte mit der Seite interagieren (Klicken, Tippen oder Taste drücken) um Audio für den Caller zu aktivieren.
        </div>
...
// Hilfsfunktion: aktiven Sound-Pool basierend auf Zufalls-Caller ermitteln
function getActiveSoundPool() {
  if (activeCallerName && config.caller.randomCaller) {
    return getActiveSoundPool().filter(s => s.callerName === activeCallerName);
  }
  return config.caller.sounds || [];
}
...
function isSoundInQueue(trigger: string): boolean {
  ...
  const matchingSounds = getActiveSoundPool().filter(sound =>
...
async function processGameData(gameData: IGameData, oldGameData: IGameData, fromWebSocket: boolean = false): Promise<void> {
  ...
  // Zufalls-Caller: Bei neuem Match einen zufälligen Caller wählen
  const currentMatchId = gameData.match.id || String(gameData.match.round);
  if (config.caller.randomCaller && gameData.match.round === 1 && gameData.match.turns[0].throws.length === 0 && gameData.match.player === 0) {
    if (lastMatchId !== currentMatchId) {
      lastMatchId = currentMatchId;
      // Alle eindeutigen Caller-Namen aus aktivierten Sounds sammeln
      const callerNames = [...new Set(
        config.caller.sounds
          .filter(s => s.enabled && s.callerName)
          .map(s => s.callerName as string),
      )];
      if (callerNames.length > 0) {
        activeCallerName = callerNames[Math.floor(Math.random() * callerNames.length)];
        console.log(`Autodarts Tools: Zufalls-Caller gewählt: "${activeCallerName}"`);
      } else {
        activeCallerName = null; // Kein Caller-Name gesetzt → alle Sounds nutzen
      }
    }
  } else if (!config.caller.randomCaller) {
  ...
  if (gameData.match.round === 1 && gameData.match.turns[0].throws.length === 0 && gameData.match.player === 0 && gameData.match.variant !== "Bull-off") {
    ...
    if (isBot) {
      ...
      const hasPlayerNameSound = getActiveSoundPool().some(sound =>
  ...
  if (gameData.match.state?.checkoutGuide?.length) {
    ...
    if (config.caller.callCheckout && currentScore > 0 && gameData.match.turns[0].throws.length === 0) {
      ...
      const hasShortSound = getActiveSoundPool().some(sound =>
      ...
      if (hasShortSound) {
        ...
        const hasSpecificSound = getActiveSoundPool().some(sound =>
    ...
    if (winner) {
      ...
      if (config.caller.callEveryDart) {
        ...
        if (throwName.toLowerCase() === "25" && throwBed === "Single") {
          const hasS25Sound = getActiveSoundPool().some(sound =>
...
function playSound(trigger: string): void {
  ...
  // Find all sounds that match the trigger (Zufalls-Caller wird in getActiveSoundPool() berücksichtigt)
  let matchingSounds = getActiveSoundPool().filter((sound) => {
  ...
  if (!matchingSounds.length && trigger.length > 1) {
    ...
    if (firstChar === "m" && !trigger.includes("_")) {
      ...
      if (!Number.isNaN(numberAfterM) && numberAfterM >= 1 && numberAfterM <= 20) {
        ...
        matchingSounds = getActiveSoundPool().filter(sound =>
      ...
      if (/^\d+$/.test(number)) {
        ...
        if (matchingSounds.length) {
          ...
          if (soundToPlay.url || soundToPlay.base64 || soundToPlay.soundId || soundToPlay.tts) {
            ...
            const numberSounds = getActiveSoundPool().filter(sound =>
