...
import { initCareerBotHint, onRemoveCareerBotHint } from "@/entrypoints/lobby.content/career-bot-hint";
import { botSkinPicker, botSkinPickerOnRemove } from "@/entrypoints/lobby.content/bot-skin-picker";
  ...
  async main(ctx: any) {
    lobbyReadyUnwatch = AutodartsToolsUrlStatus.watch(async (url: string) => {
      ...
      if (/\/lobbies\/(?!.*new\/)/.test(url)) {
        ...
        // v2.9.51: Bot-Hinweis-Overlay für Karriere/Turnier-Matches
        initCareerBotHint(url).catch(console.error);

        // v2.9.78: PDC Bot-Skin-Picker (funktioniert auch außerhalb Career/Turnier)
        botSkinPicker().catch(console.error);
