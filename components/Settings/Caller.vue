<template>
  <template v-if="!$attrs['data-feature-index']">
      ...
      <div class="relative z-10 flex h-full flex-col justify-between">
        <div>
          <h3 class="mb-1 flex flex-col items-start gap-2 font-bold uppercase md:flex-row md:items-center md:justify-between">
            <span>Einstellungen – Caller</span>
            <div class="flex w-full flex-wrap gap-2 md:w-auto">
              <AppButton @click="sortSoundsByTriggers" size="sm" class="!py-1 text-xs md:text-sm" auto title="Sounds nach Triggern sortieren">
                <span class="icon-[pixelarticons--sort-alphabetic] mr-1" />
                <span class="whitespace-nowrap">Sortieren</span>
              </AppButton>
              <AppButton @click="openDeleteAllModal" size="sm" class="!py-1 text-xs md:text-sm" auto type="danger" title="Alle Sounds löschen">
                <span class="icon-[pixelarticons--trash] mr-1" />
                <span class="whitespace-nowrap">Alle löschen</span>
              </AppButton>
              <AppButton @click="openTTSModal()" size="sm" class="!py-1 text-xs md:text-sm" auto type="warning" :disabled="!isTTSAvailable" title="Sound aus Text-to-Speech generieren">
                <span class="icon-[pixelarticons--chat] mr-1" />
                <span class="whitespace-nowrap">TTS generieren</span>
              </AppButton>
              <AppButton @click="openImportURLModal" size="sm" class="!py-1 text-xs md:text-sm" auto type="success">
                ...
                <span class="whitespace-nowrap">Von URL importieren</span>
              </AppButton>
              <AppButton @click="openUploadModal" size="sm" class="!py-1 text-xs md:text-sm" auto type="success">
                ...
                <span class="whitespace-nowrap">Dateien hochladen</span>
              </AppButton>
            </div>
          </h3>
          <div class="space-y-3 text-white/70">
            <p>Konfiguriere den Caller. Klicke auf das Plus-Symbol, um einen neuen Sound hinzuzufügen.<br>Klicke auf <b>Von URL importieren</b>, um vordefinierte Caller-Sets zu importieren.</p>

            <div class="flex flex-col gap-20 sm:flex-row">
              <div class="mt-2 flex items-center gap-2">
                <div class="flex items-center gap-2">
                  <span>Jeden Dart ansagen</span>
                </div>
                <AppToggle
              ...
              <div class="mt-2 flex items-center gap-2" title="Wenn aktiv, wird der eingebaute Autodarts-Caller (Score-Ansagen von play.autodarts.io) automatisch stumm geschaltet, sobald einer der eigenen Sound-Module (Caller / Sound FX / Crowd) aktiviert ist. Verhindert doppelte Ansagen.">
                <div class="flex items-center gap-2">
                  <span data-testid="mute-native-autodarts-label">Nativen Autodarts-Caller stummschalten</span>
                </div>
                <AppToggle
                  @update:model-value="config.caller.muteNativeAutodarts = !(config.caller.muteNativeAutodarts !== false)"
                  :model-value="config.caller.muteNativeAutodarts !== false"
                  data-testid="mute-native-autodarts-toggle"
                />
              </div>
            </div>
            ...
            <div class="mt-2 flex items-center gap-2 text-sm">
              ...
              <p>Ziehe Sounds per Drag & Drop, um ihre Reihenfolge zu ändern</p>
            </div>
              ...
              >
                ...
                >
                  <div class="truncate border-b border-white/30 pb-1 font-mono text-xxs" :title="sound.tts ? `TTS: ${sound.tts.text}` : sound.url">
                    {{ sound.tts ? `TTS: "${sound.tts.text}"` : (sound.url || "Hochgeladen") }}
                  </div>
                  <div class="mt-1 truncate font-mono uppercase">
                    {{ Array.isArray(sound.triggers) ? sound.triggers.join(', ') : 'Keine Trigger' }}
                  </div>
    ...
    <AppModal
      ...
      :show="showSoundModal"
      :title="isEditMode ? 'Sound bearbeiten' : 'Sound hinzufügen'"
    >
      <div class="space-y-4">
        <div>
          ...
          <div class="relative">
            ...
            <AppInput
              ...
              type="text"
              placeholder="Name für diesen Sound eingeben"
              class="pl-9"
            />
          </div>
        </div>
        ...
        <div v-if="!newSound.base64">
          ...
          <div class="relative">
            ...
            <AppInput
              ...
              type="url"
              placeholder="https://beispiel.de/sound.mp3"
              class="pl-9"
            />
          </div>
    ...
    >
      <div class="space-y-4">
        ...
        >
          ...
          <p class="text-white/70">
            Sound-Dateien hier ablegen oder klicken zum Durchsuchen
          </p>
        ...
        <div class="mt-4 space-y-3">
          <div class="flex items-center">
            <label class="flex cursor-pointer items-center">
              ...
              >
              <span class="ml-2 text-sm">Trigger aus Dateinamen generieren</span>
            </label>
            <span
              class="icon-[pixelarticons--info-box] ml-2 cursor-help text-white/50"
              title="Wenn aktiviert, werden Trigger automatisch aus den Dateinamen generiert. Beispiel: 180.mp3 löst bei einem 180er-Wurf aus."
            />
          </div>
          <div v-if="!generateTriggersFromFilenames">
            <label for="bulk-trigger" class="mb-1 block text-sm font-medium text-white">
              Gleichen Trigger für alle Dateien verwenden (optional)
            </label>
            <div class="relative">
              <span class="absolute inset-y-0 left-3 flex items-center text-white/60">
              ...
              <AppInput
                ...
                v-model="bulkTrigger"
                type="text"
                placeholder="z.B. t20, 180, gameshot"
                class="pl-9"
              />
            </div>
            <p class="mt-1 text-xs text-white/60">
              Wenn angegeben, wird dieser Trigger allen hochgeladenen Dateien zugewiesen.
            </p>
          </div>
        </div>
      </div>
      ...
      <template #footer>
        ...
        >
          {{ selectedFiles.length }} Dateien speichern
        </AppButton>
      </template>
    </AppModal>
    ...
    <AppModal
      ...
      title="Sounds von URL importieren"
    >
      <div class="space-y-4">
        <div>
          <label for="preset-url" class="mb-1 block text-sm font-medium text-white">Vordefinierte Caller-Sets (optional)</label>
          <div class="relative">
            ...
            />
            <button
              v-if="selectedPresetURL"
              type="button"
              @click="playVoicePreview"
              :disabled="isPreviewPlaying"
              class="absolute inset-y-0 right-2 my-1 px-3 rounded bg-emerald-500/20 border border-emerald-400/40 text-emerald-300 text-xs font-bold uppercase tracking-widest disabled:opacity-50"
              title="Kurze Sprachprobe abspielen (Browser-TTS)"
              data-testid="voice-pack-preview-btn"
            >
              {{ isPreviewPlaying ? '■ Stop' : '▶ Anhören' }}
            </button>
          </div>

          <p class="mt-2 text-xs text-white/60">
            <b class="text-emerald-300">▶ Anhören</b> nutzt die Browser-Sprachausgabe — Sprache & Geschlecht entsprechen dem Voice-Pack, die exakte Sprecherstimme hörst du erst nach dem Import. Einige vordefinierte Caller-Sets funktionieren in Safari möglicherweise nicht.
          </p>
        </div>

        <div>
          <label for="base-url" class="mb-1 block text-sm font-medium text-white">Basis-URL</label>
          <div class="relative">
            <span class="absolute inset-y-0 left-3 flex items-center text-white/60">
              <span class="icon-[pixelarticons--link]" />
            </span>
            <AppInput
              ...
              v-model="baseURL"
              type="url"
              placeholder="https://beispiel.de/sounds"
              class="pl-9"
            />
          </div>
          <p class="mt-2 text-xs text-white/60">
            Gib eine URL zu einem Sound-Verzeichnis an. Das Skript prüft dann automatisch nach passenden Dateien und Triggern.
          </p>
          <div v-if="urlError" class="mt-1 text-sm text-red-500">
            {{ urlError }}
          </div>
          ...
          <div class="space-y-3 rounded-md bg-white/5 p-3">
            ...
            <div v-if="isDownloadingZip">
              <div class="mb-1 flex items-center justify-between">
                <span class="text-xs">ZIP-Datei wird via Hintergrund-Service heruntergeladen…</span>
            ...
            <div v-if="isExtractingZip">
              <div class="mb-1 flex items-center justify-between">
                <span class="text-xs">ZIP-Inhalte werden entpackt…</span>
            ...
            <div v-if="isProcessingCsv">
              <div class="mb-1 flex items-center justify-between">
                <span class="text-xs">Sound-Zuordnung wird verarbeitet…</span>
        ...
        <div v-else-if="isImporting">
          <div class="mb-2 flex items-center justify-between">
            <span class="text-sm">Sounds werden importiert…</span>
            <span class="text-sm">{{ importedCount }} gefunden</span>
          </div>
          <div class="h-2 overflow-hidden rounded-full bg-white/10">
            <div
    ...
    <AppModal
      ...
      title="Alle Sounds löschen"
    >
      <div class="space-y-4">
        <p>Möchtest du wirklich alle Sounds löschen? Diese Aktion kann nicht rückgängig gemacht werden.</p>
        <p class="text-sm text-white/60">
          Damit werden alle {{ config?.caller.sounds.length || 0 }} Sounds aus dem Caller entfernt.
        </p>
      </div>
    ...
    <AppModal
      @close="closeTTSModal"
      :show="showTTSModal"
      title="TTS-Sound generieren"
    >
      <div class="space-y-4">
        <div>
          <label for="tts-text-caller" class="mb-1 block text-sm font-medium text-white">Zu sprechender Text</label>
          <div class="relative">
            <span class="absolute inset-y-0 left-3 flex items-center text-white/60">
            ...
            <AppInput
              ...
              type="text"
              placeholder="z.B. Einhundertachtzig!"
              class="pl-9"
            />
          </div>
        </div>
        ...
        <div>
          <label class="mb-1 block text-sm font-medium text-white">Stimme</label>
          <AppSelect
        ...
        <div>
          <label class="mb-1 block text-sm font-medium text-white">
            Geschwindigkeit <span class="text-xs text-white/60">({{ ttsForm.rate.toFixed(1) }}x)</span>
          </label>
          <AppSlider
        ...
        <div>
          <label class="mb-1 block text-sm font-medium text-white">
            Tonhöhe <span class="text-xs text-white/60">({{ ttsForm.pitch.toFixed(1) }})</span>
          </label>
          <AppSlider
      ...
      <div class="relative z-10 flex h-full flex-col justify-between">
        <div>
          ...
          <p class="w-2/3 text-white/70">
            Sagt Punkte, Checkouts und besondere Ereignisse während deines Matches an – mit individuellen Sound-Effekten.
          </p>
        </div>
        <div class="flex">
...
async function playVoicePreview() {
  ...
  if (!selectedPresetURL.value) return;
  isPreviewPlaying.value = true;
  const res = await previewVoicePack(selectedPresetURL.value);
  if (!res) {
    isPreviewPlaying.value = false;
    showNotification("Browser-Sprachausgabe nicht verfügbar", "error");
    return;
  }
  // Auto-Reset nach Ende (kein reliable "end"-event über Browser hinweg → Timeout)
  setTimeout(() => { isPreviewPlaying.value = false; }, 4500);
}
...
// Predefined caller sets - v2.9.50: Sortiert nach Empfehlung (Deutsch/Englisch zuerst mit Flaggen)
// Alle Voice-Packs stammen vom offiziellen lbormann/darts-caller Projekt via peschi.org.
const callerSets = [
  { value: "", label: "— Caller auswählen (optional) —" },

  // ⭐ EMPFOHLEN: Deutsch
  { value: "https://darts-downloads.peschi.org/soundfiles/de-DE-Vicki-Female-v8.zip",  label: "🇩🇪 Vicki (weiblich) — Klassisch Deutsch" },
  { value: "https://darts-downloads.peschi.org/soundfiles/de-DE-Daniel-Male-v8.zip",   label: "🇩🇪 Daniel (männlich) — Ernster Sprecher DE" },
  { value: "https://darts-downloads.peschi.org/soundfiles/de-AT-Hannah-Female-v5.zip", label: "🇦🇹 Hannah (weiblich) — Österreichisch DE" },

  // ⭐ EMPFOHLEN: Britisches Englisch (PDC-Feeling)
  { value: "https://darts-downloads.peschi.org/soundfiles/en-GB-Arthur-Male-v4.zip",   label: "🏴󠁧󠁢󠁥󠁮󠁧󠁿 Arthur (männlich) — PDC-Style EN" },
  { value: "https://darts-downloads.peschi.org/soundfiles/en-GB-Amy-Female-v4.zip",    label: "🏴󠁧󠁢󠁥󠁮󠁧󠁿 Amy (weiblich) — Britisch EN" },

  // Amerikanisches Englisch
  { value: "https://darts-downloads.peschi.org/soundfiles/en-US-Stephen-Male-v8.zip",  label: "🇺🇸 Stephen (männlich) — Sportlich US" },
  { value: "https://darts-downloads.peschi.org/soundfiles/en-US-Matthew-Male-v6.zip",  label: "🇺🇸 Matthew (männlich)" },
  { value: "https://darts-downloads.peschi.org/soundfiles/en-US-Joey-Male-v9.zip",     label: "🇺🇸 Joey (männlich) — Jung/energetisch" },
  { value: "https://darts-downloads.peschi.org/soundfiles/en-US-Kevin-Male-v5.zip",    label: "🇺🇸 Kevin (männlich)" },
  { value: "https://darts-downloads.peschi.org/soundfiles/en-US-Justin-Male-v5.zip",   label: "🇺🇸 Justin (männlich)" },
  { value: "https://darts-downloads.peschi.org/soundfiles/en-US-Gregory-Male-v6.zip",  label: "🇺🇸 Gregory (männlich)" },
  { value: "https://darts-downloads.peschi.org/soundfiles/en-US-Ivy-Female-v8.zip",    label: "🇺🇸 Ivy (weiblich)" },
  { value: "https://darts-downloads.peschi.org/soundfiles/en-US-Joanna-Female-v9.zip", label: "🇺🇸 Joanna (weiblich)" },
  { value: "https://darts-downloads.peschi.org/soundfiles/en-US-Danielle-Female-v6.zip", label: "🇺🇸 Danielle (weiblich)" },
  { value: "https://darts-downloads.peschi.org/soundfiles/en-US-Kimberly-Female-v5.zip", label: "🇺🇸 Kimberly (weiblich)" },
  { value: "https://darts-downloads.peschi.org/soundfiles/en-US-Ruth-Female-v5.zip",   label: "🇺🇸 Ruth (weiblich)" },
  { value: "https://darts-downloads.peschi.org/soundfiles/en-US-Salli-Female-v5.zip",  label: "🇺🇸 Salli (weiblich)" },
  { value: "https://darts-downloads.peschi.org/soundfiles/en-US-Kendra-Female-v9.zip", label: "🇺🇸 Kendra (weiblich)" },

  // Weitere Sprachen
  { value: "https://darts-downloads.peschi.org/soundfiles/nl-NL-Laura-Female-v5.zip",  label: "🇳🇱 Laura (weiblich) — Niederländisch" },
  { value: "https://darts-downloads.peschi.org/soundfiles/fr-FR-Remi-Male-v3.zip",     label: "🇫🇷 Remi (männlich) — Französisch" },
  { value: "https://darts-downloads.peschi.org/soundfiles/fr-FR-Lea-Female-v3.zip",    label: "🇫🇷 Lea (weiblich) — Französisch" },
  { value: "https://darts-downloads.peschi.org/soundfiles/es-ES-Sergio-Male-v3.zip",   label: "🇪🇸 Sergio (männlich) — Spanisch" },
  { value: "https://darts-downloads.peschi.org/soundfiles/es-ES-Lucia-Female-v3.zip",  label: "🇪🇸 Lucia (weiblich) — Spanisch" },
];
...
onMounted(async () => {
  ...
  try {
    ...
    if (queuedUrl && typeof queuedUrl === "string" && queuedUrl.startsWith("https://")) {
      ...
      baseURL.value = queuedUrl;
      showImportURLModal.value = true;
      showNotification("Voice-Pack aus Match übernommen — Import startet …");
      setTimeout(() => {
        fetchSoundsFromURL().catch((err) => console.error("[Caller] Auto-Import fehlgeschlagen:", err));
      }, 400);
    }
  } catch (err) {
    console.error("[Caller] Voice-Pack-Queue konnte nicht verarbeitet werden:", err);
  }
});
