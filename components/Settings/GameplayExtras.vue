      ...
      <div style="background:#0a1520; border-radius:6px; padding:16px; border:1px solid #1e3a5f;">
        <div style="display:flex; align-items: center; justify-content:space-between; margin-bottom:12px;">
          <div>
            ...
            <div style="font-size:12px; color:#8899aa; margin-top:2px;">Herzschlag-Modus bei Match-Dart auf Doppel</div>
          </div>
          <input type="checkbox" v-model="config.clutch.enabled"
            style="width:22px; height:22px; accent-color:#E8002D; cursor:pointer;" />
        </div>

        <div v-if="config.clutch.enabled" style="display:flex; flex-direction:column; gap:10px; padding-top:10px; border-top:1px solid #1e3a5f;">
      ...
      <div style="background:#0a1520; border-radius:6px; padding:16px; border:1px solid #1e3a5f;">
        <div style="display:flex; align-items: center; justify-content:space-between; margin-bottom:12px;">
          <div>
            ...
            <div style="font-size:12px; color:#8899aa; margin-top:2px;">Verfolgt automatisch deine Bilanz gegen Freunde</div>
          </div>
          <input type="checkbox" v-model="config.rivalry.enabled"
            style="width:22px; height:22px; accent-color:#E8002D; cursor:pointer;" />
        </div>

        <div v-if="config.rivalry.enabled" style="display:flex; flex-direction:column; gap:10px; padding-top:10px; border-top:1px solid #1e3a5f;">
          <label style="display:flex; align-items: center; justify-content:space-between; cursor:pointer;">
            <div>
          ...
          <div style="margin-top:8px;">
            <div style="font-size:12px; color:#F5C842; letter-spacing:1px; text-transform:uppercase; margin-bottom:8px;">
              Deine Rivaleitäten
            </div>

            <div v-if="isLoadingRivaleries" style="font-size:13px; color:#556677; text-align: center; padding:12px;">
              ⏳ Lade Rivaleitäten...
            </div>

            <div v-else-if="rivalries.length === 0"
              style="font-size:13px; color:#556677; text-align: center; padding:12px; border:1px dashed #1e3a5f; border-radius:4px;">
            ...
            <div v-else style="display:flex; flex-direction:column; gap:8px;">
              <div v-for="r in rivalries" :key="r.opponentId"
                ...
                <div v-if="getStreakLabel(r.streak)" style="font-size:11px; color:#F5C842; margin-top:6px;">
                  {{ getStreakLabel(r.streak) }}
                </div>
                <div v-if="r.trophyHolder" style="font-size:11px; color:#F5C842; margin-top:4px;">
            ...
            <!-- Zurücksetzen Button -->
            <div v-if="rivalries.length > 0" style="margin-top:10px;">
      ...
      <div style="background:#0a1520; border-radius:6px; padding:16px; border:1px solid #1e3a5f;">
        <div style="display:flex; align-items: center; justify-content:space-between; margin-bottom:12px;">
          <div>
            ...
            <div style="font-size:12px; color:#8899aa; margin-top:2px;">Gleicht Stärkeunterschiede automatisch aus</div>
          </div>
          <input type="checkbox" v-model="config.handicap.enabled"
            style="width:22px; height:22px; accent-color:#E8002D; cursor:pointer;" />
        </div>

        <div v-if="config.handicap.enabled" style="display:flex; flex-direction:column; gap:10px; padding-top:10px; border-top:1px solid #1e3a5f;">

          <!-- Handicap-Typ -->
          <div>
            <div style="font-size:12px; color:#8899aa; letter-spacing:1px; text-transform:uppercase; margin-bottom:8px;">Handicap-Typ</div>
            <div style="display:flex; flex-direction:column; gap:6px;">
              <label v-for="opt in [
                ...
                style="display:flex; align-items:center; gap:10px; padding:8px 10px; border-radius:4px; cursor:pointer;"
          ...
          <!-- Info-Box -->
          <div style="background:#0D1B2A; border-left:3px solid #F5C842; padding:10px 12px; border-radius:0 4px 4px 0; font-size:12px; color:#8899aa; line-height:1.6;">
