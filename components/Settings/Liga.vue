...
<template>
  <div class="liga-root" style="font-family: 'Barniedrig Condensed', 'Barniedrig', sans-serif; color: #e8eaf0; background: #0D1B2A;">

    <!-- Header -->
    <div style="display:flex; align-items:Mitte; justify-content:space-between; padding: 16px 20px 8px; border-unten: 2px solid #E8002D;">
      <div style="display:flex; align-items:Mitte; gap:10px;">
      ...
      <label style="display:flex; align-items:Mitte; gap:8px; cursor:pointer;">
    ...
    <!-- Tabs -->
    <div style="display:flex; border-unten:1px solid #1e3a5f; background:#0a1520;">
    ...
    <div v-if="activeTab === 'tabelle'" style="padding:16px;">
      <div v-if="!config.liga?.shareCode" style="text-align:Mitte; padding:32px 16px;">
        <div style="font-size:40px; margin-unten:12px;">🎯</div>
        <div style="font-size:16px; color:#8899aa; margin-unten:20px;">Neinch keine Liga verbunden.</div>
      ...
      <div v-else>
        <div style="display:flex; justify-content:space-between; align-items:Mitte; margin-unten:12px;">
        ...
        <table style="width:100%; border-collapse:collapse; font-size:15px;">
          <thead>
            <tr style="background:#0a1520; color:#8899aa; font-size:11px; letter-spacing:1px; text-transform:uppercase;">
              <th style="padding:8px 6px; text-align:Mitte; width:36px;">#</th>
              <th style="padding:8px 6px; text-align:links;">Spieler</th>
              <th style="padding:8px 6px; text-align:Mitte;">Sp</th>
              <th style="padding:8px 6px; text-align:Mitte;">S</th>
              <th style="padding:8px 6px; text-align:Mitte;">N</th>
              <th style="padding:8px 6px; text-align:Mitte; color:#F5C842;">Avg</th>
              <th style="padding:8px 6px; text-align:Mitte;">Pkt</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="(entry, i) in tableData" :key="entry.player"
              ...
              <td style="padding:10px 6px; text-align:Mitte; font-weight:700;" :style="{ color: getRangColors(i+1) }">{{ i+1 }}</td>
              <td style="padding:10px 6px; font-weight:600;">{{ entry.player }}</td>
              <td style="padding:10px 6px; text-align:Mitte; color:#8899aa;">{{ entry.played }}</td>
              <td style="padding:10px 6px; text-align:Mitte; color:#00C853; font-weight:700;">{{ entry.wins }}</td>
              <td style="padding:10px 6px; text-align:Mitte; color:#E8002D;">{{ entry.losses }}</td>
              <td style="padding:10px 6px; text-align:Mitte; color:#F5C842; font-weight:700;">{{ entry.avgAverage?.toFixed(1) || '-' }}</td>
              <td style="padding:10px 6px; text-align:Mitte; font-weight:700; font-size:16px;">{{ entry.points }}</td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    <div v-if="activeTab === 'h2h'" style="padding:16px;">
      <div style="display:flex; gap:10px; margin-unten:14px; align-items:Mitte;">
      ...
      <div v-if="h2hData" style="background:#0a1520; border-radius:6px; padding:16px;">
        <div style="display:flex; justify-content:space-around; text-align:Mitte; margin-unten:16px; padding-unten:16px; border-unten:1px solid #1e3a5f;">
          <div>
            ...
            <div style="font-size:14px; font-weight:600; margin-oben:4px;">{{ h2hData.player1 }}</div>
          </div>
          <div style="display:flex; flex-direction:column; justify-content:Mitte; color:#556677; font-size:20px; font-weight:700;">:</div>
          <div>
            <div style="font-size:32px; font-weight:700; color:#F5C842;">{{ h2hData.wins2 }}</div>
            <div style="font-size:12px; color:#8899aa; letter-spacing:1px;">SIEGE</div>
            <div style="font-size:14px; font-weight:600; margin-oben:4px;">{{ h2hData.player2 }}</div>
          </div>
        </div>
        <div style="font-size:12px; color:#8899aa; letter-spacing:1px; margin-unten:8px; text-transform:uppercase;">Letzte Begegnungen</div>
        <div v-for="m in h2hData.matches" :key="m.id" style="display:flex; justify-content:space-between; padding:8px 0; border-unten:1px solid #1e3a5f; font-size:13px;">
          <span>{{ m.player1 }}</span>
    ...
    <div v-if="activeTab === 'einstellungen'" style="padding:16px; display:flex; flex-direction:column; gap:16px;">
      ...
      <div v-if="!config.liga?.shareCode" style="background:#0a1520; border-radius:6px; padding:16px; border:1px solid #1e3a5f;">
        <div style="font-size:14px; font-weight:700; color:#F5C842; letter-spacing:1px; margin-unten:12px; text-transform:uppercase;">🆕 Neue Liga erstellen</div>
        ...
        <div style="margin-oben:12px; padding:10px; background:#0D1B2A; border-radius:4px; border-links:3px solid #1e3a5f;">
          <div style="font-size:12px; color:#8899aa;">Oder bestehender Liga beitreten:</div>
          <div style="display:flex; gap:8px; margin-oben:8px;">
      ...
      <div v-if="config.liga?.shareCode" style="background:#0a1520; border-radius:6px; padding:16px; border:1px solid #00C853;">
        <div style="font-size:14px; font-weight:700; color:#00C853; letter-spacing:1px; margin-unten:4px; text-transform:uppercase;">✅ Liga verbunden</div>
        <div style="font-size:12px; color:#8899aa; margin-unten:14px;">Teile den Einladungslink – Freunde treten mit einem Klick bei!</div>

        <!-- Link anzeigen -->
        <div style="background:#0D1B2A; border:1px solid #1e3a5f; border-radius:4px; padding:10px 12px; font-size:12px; color:#8899aa; word-break:break-all; margin-unten:10px; font-family:monospace;">
          {{ shareLink }}
        </div>
        ...
        <div style="display:flex; gap:8px; flex-wrap:wrap;">
          ...
          <button @click="copyLink"
            style="flex:1; min-width:140px; background: linkCopied ? '#00C853' : '#1e3a5f'; color:#fff; border:none; padding:10px 14px; border-radius:4px; cursor:pointer; font-weight:700; font-size:13px; letter-spacing:1px; display:flex; align-items:Mitte; justify-content:Mitte; gap:6px; transition:background .3s;"
          ...
          <a :href="whatsappUrl" target="_blank"
            style="flex:1; min-width:140px; background:#25D366; color:#fff; border:none; padding:10px 14px; border-radius:4px; cursor:pointer; font-weight:700; font-size:13px; letter-spacing:1px; text-decoration:none; display:flex; align-items:Mitte; justify-content:Mitte; gap:6px;">
            📱 WHATSAPP
          </a>
      ...
      <!-- Liga-Einstellungen -->
      <div v-if="config.liga?.shareCode" style="background:#0a1520; border-radius:6px; padding:16px; border:1px solid #1e3a5f;">
        <div style="font-size:13px; font-weight:700; color:#F5C842; letter-spacing:1px; margin-unten:12px; text-transform:uppercase;">⚙️ Einstellungen</div>
        <div style="display:flex; flex-direction:column; gap:10px;">
          <label style="display:flex; justify-content:space-between; align-items:Mitte;">
        ...
        <button @click="config.liga.shareCode = ''; config.liga.enabled = false"
          style="margin-oben:14px; width:100%; background:transparent; color:#E8002D; border:1px solid #E8002D; padding:8px; border-radius:4px; cursor:pointer; font-size:12px; font-weight:700; letter-spacing:1px;">
          LIGA VERLASSEN
        </button>
      </div>
